...
parser.add_option('--interface',
    ...
    default='182.162.90.79', help='Set the IP or hostname for the proxy to listen on')
...
cmds = [['./tools/compile-handlebars-templates', 'forever'],
        ['python', 'manage.py', 'rundjango'] +
          manage_args + ['182.162.90.79:%d' % (django_port,)],
        ['python', '-u', 'manage.py', 'runtornado'] +
          manage_args + ['182.162.90.79:%d' % (tornado_port,)],
        ['./tools/run-dev-queue-processors'] + manage_args,
        ['env', 'PGHOST=182.162.90.79', # Force password authentication using .pgpass
...
class Resource(resource.Resource):
    def getChild(self, name, request):
        ...
        if (request.uri in ['/json/get_events'] or
            ...
            return proxy.ReverseProxyResource('182.162.90.79', tornado_port, '/'+name)

        elif (request.uri.startswith('/webpack') or
              request.uri.startswith('/socket.io')):
            return proxy.ReverseProxyResource('182.162.90.79', webpack_port, '/'+name)

        return proxy.ReverseProxyResource('182.162.90.79', django_port, '/'+name)
